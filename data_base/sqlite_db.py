...
def sql_start():
    ...
    base.execute("CREATE TABLE IF NOT EXISTS book(name_room TEXT, date_book TEXT, time_book TEXT, name_client TEXT, fam_client TEXT, org_client TEXT)")
    base.commit()
...
def sql_delete_room(name_room):
    name_room = name_room
    cur.execute(f"DELETE FROM rooms WHERE name={name_room}")
    base.commit()
...
async def sql_book_copy_nameclient():
